...
- **Direct Insert Mode**: User focuses any editable field (URL bar, Gmail composer, doc comment) → activates Echo mic → live transcription streams into side-panel editor and, with user consent, mirrors into the active DOM element so text appears where they are typing; summarize/rewriter actions can then replace or append within that field.
...
- Floating Echo widget injected via content script on every page (toggleable from settings) with quick-record controls, mode switch, prompt editor, and re-generate action.
...
- Floating Echo icon in-page (bottom-right by default, dismissible from settings) opens a compact popup with mic control, mode toggle (syncs with side panel), prompt textbox (compose only), settings launcher, and re-generate button.
...
- Branding: Echo palette anchors on primary `#5968F2` for call-to-action elements and accent `#AEB6FF` for backgrounds, focus rings, and hover states to keep the interface cohesive.
...
- Manifest V3 extension with:
  ...
  - Content-script overlay that renders the floating Echo widget inside each frame.
...
8. Polish UX: keyboard shortcuts, onboarding, accessibility, floating widget styling, and Echo theming anchored in `#5968F2`/`#AEB6FF`.
